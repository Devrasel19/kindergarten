...
if (!class_exists('Kidsa_Post_Column_Customize')){
    class Kidsa_Post_Column_Customize{
        //$instance variable
        ...
        public function edit_event_columns($columns){
            ...
            $columns['title'] = esc_html__('Title','kidsa-core');
            $columns['thumbnail'] = '<a href="edit.php?post_type=event&orderby=title&order='.urlencode($order).'">'.esc_html__('Thumbnail','kidsa-core').'</a>';
            ...
            $columns['icon'] = esc_html__('Icon','kidsa-core');
            $columns['date'] = esc_html__('Date','kidsa-core');
            return $columns;
        }
        ...
        public function add_event_thumbnail_columns($column,$post_id) {
            switch ( $column ) {
                case 'thumbnail' :
                    ...
                    break;
                case 'icon' :
                    $event_meta_option = get_post_meta($post_id ,'kidsa_event_options', true);
        ...
        public function add_event_category_columns($string,$columns,$post_id){
            $post_term_meta = get_term_meta($post_id,'kidsa_event_category',true);
        ...
        public function edit_project_columns($columns){
            ...
            $columns['cb'] = '<input type="checkbox" />';
            $columns['title'] = esc_html__('Title','kidsa-core');
            $columns['thumbnail'] = '<a href="edit.php?post_type=project&orderby=title&order='.urlencode($order).'">'.esc_html__('Thumbnail','kidsa-core').'</a>';
        ...
        public function add_project_category_columns($string,$columns,$post_id){
            $post_term_meta = get_term_meta($post_id,'kidsa_project_category',true);
            $icon = isset($post_term_meta['icon']) ? $post_term_meta['icon'] : '';
            switch ( $columns ) {
                case 'icon' :
                    echo '<i class="neaterller-font-size50 '.$icon.'"></i>';
        ...
        public function edit_team_columns($columns){
            ...
            $columns['cb'] = '<input type="checkbox" />';
            $columns['title'] = esc_html__('Title','kidsa-core');
            $columns['thumbnail'] = '<a href="edit.php?post_type=team&orderby=title&order='.urlencode($order).'">'.esc_html__('Thumbnail','kidsa-core').'</a>';
        ...
        public function add_team_category_columns($string,$columns,$post_id){
            $post_term_meta = get_term_meta($post_id,'kidsa_team_category',true);
            $icon = isset($post_term_meta['icon']) ? $post_term_meta['icon'] : '';
            switch ( $columns ) {
                case 'icon' :
                    echo '<i class="neaterller-font-size50 '.$icon.'"></i>';
    ...
    if ( class_exists('Kidsa_Post_Column_Customize')){
        Kidsa_Post_Column_Customize::getInstance();
    }
}