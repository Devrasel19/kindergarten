...
class Kidsa_Tags_Widget extends WP_Widget
{
    ...
    public function __construct()
    {
        parent::__construct(
            'kidsa_tags',
            esc_html__('Kidsa: Tags', 'kidsa-core'),
...
//            'type' => $post_type,
        ...
        if (!empty($tags)):?>
            ...
            esc_html__(' Oops, there are no tags.', 'kidsa-core')
            ?>
        <?php endif; ?>
        </div>
        <?php
    ...
    public function form($instance)
    {
        ...
        if (!empty($instance) && $instance['title']) {
            ...
            $title = esc_html__('Tags', 'kidsa-core');

        }
        $order = !empty($instance) && $instance['order'] ? $instance['order'] : 'ASC';
        $orderby = !empty($instance) && $instance['orderby'] ? $instance['orderby'] : 'ID';
        $post_tags = array(
            'post_tag' => esc_html__('Blog', 'kidsa-core'),
            'packages-tag' => esc_html__('Packages', 'kidsa-core'),
            'project-tag' => esc_html__('Project', 'kidsa-core'),
            'course-tag' => esc_html__('Courses', 'kidsa-core'),
            'service-tag'=> esc_html__('Service', 'kidsa-core')
        );
        $order_arr = array(
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('post_tags')) ?>"><?php esc_html_e('Post Tags', 'kidsa-core'); ?></label>
...
if (!function_exists('Kidsa_Tags_Widget')) {
    function Kidsa_Tags_Widget()
    {
        register_widget('Kidsa_Tags_Widget');
    }

    add_action('widgets_init', 'Kidsa_Tags_Widget');
}