...
class Kidsa_Recent_Post_title_Widget extends WP_Widget

{
    public function __construct()
    {
        parent::__construct(
            'kidsa_popular_posts_title',
            esc_html__('Kidsa: Recent Posts Title', 'kidsa-core'),
            array('description' => esc_html__('Display your recent posts, with a thumbnail.', 'kidsa-core'))
        );
    }
    ...
    {   
        ...
        if ($recent_articles->have_posts()) : ?>
            ...
            esc_html__(' Oops, there are no posts.', 'kidsa-core')
            ?>
        <?php endif; ?>
        </ul>
        <?php
...

    {
        ...
        if (!empty($instance) && $instance['title']) {
            ...
            $title = esc_html__('Recent Posts', 'kidsa-core');
        ...
        $post_type = array(
            'post' => esc_html__('Blog Post Type', 'kidsa-core'),
            'packages' => esc_html__('Packages Post Type', 'kidsa-core'),
            'project' => esc_html__('project Post Type', 'kidsa-core'),
            'service' => esc_html__('Service Post Type', 'kidsa-core'),
            'courses' => esc_html__('Courses Post Type', 'kidsa-core')
        );

        $order_arr = array(
            'ASC' => esc_html__('Acceding', 'kidsa-core'),
        ...
        $orderby_arr = array(
            ...
            'comment_count' => esc_html__('Most Comment', 'kidsa-core')
        );

        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('post_type')) ?>"><?php esc_html_e('Post Type', 'kidsa-core'); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'kidsa-core'); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('no_of_posts')) ?>"><?php esc_html_e('No Of Posts', 'kidsa-core'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('order')) ?>"><?php esc_html_e('Order', 'kidsa-core'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('orderby')) ?>"><?php esc_html_e('OrderBy', 'kidsa-core'); ?></label>
...
if (!function_exists('Kidsa_Recent_Post_title_Widget')) {

    function Kidsa_Recent_Post_title_Widget()

    {
        register_widget('Kidsa_Recent_Post_title_Widget');
    }

    add_action('widgets_init', 'Kidsa_Recent_Post_title_Widget');
